...
from scrapers.form4 import fetch_form4
from transformers.normalize import normalize_form4_row

JOB_NAME = "fetch_form4"


def run() -> None:
    """
    Incremental Form 4 insider trade ingestion job.
    Fetches SEC EDGAR Form 4 filings, validates, deduplicates, and loads to DB.
    ...
    try:
        all_raw = fetch_form4(since)
        log(f"Fetched {len(all_raw)} raw rows", job=JOB_NAME)

        valid_trades = []
        valid_raws = []
        for raw in all_raw:
            normalized = normalize_form4_row(raw, job=JOB_NAME)
        ...
        if valid_trades:
            ins, skip = load_insider_trades(valid_trades, valid_raws, job=JOB_NAME)
            rows_inserted += ins
            rows_skipped += skip
